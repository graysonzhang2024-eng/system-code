...
class TestValidateTask(unittest.TestCase):
    ...
    def test_today_date_must_be_iso_date(self):
        validate_task(_base_task(today_date="2026-08-12"))
        validate_task(_base_task(today_date=""))
        with self.assertRaisesRegex(ValidationError, "today_date"):
            validate_task(_base_task(today_date="今天"))
