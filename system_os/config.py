...
def get_value(key: str, default: str = "") -> str:
    """公开的通用配置读取入口，供可插拔模块读取非敏感配置键。"""
    return _get(key, default)


def get_choice(key: str, choices: set[str], default: str = "") -> str:
    """读取一个枚举配置,忽略空值和非法值。

    每个来源都先通过校验再参与优先级:真实环境变量中的合法值优先,
    其次是 .env 中的合法值。这样临时环境变量写错时不会遮住 .env 里
    已经配置好的安全值,两个来源都不可用时再返回 ``default``。

    返回值会去除首尾空白并转成小写;``choices`` 也按同样规则比较。
    """
    normalized_choices = {choice.strip().lower() for choice in choices}
    for raw_value in (os.environ.get(key), _DOTENV.get(key)):
