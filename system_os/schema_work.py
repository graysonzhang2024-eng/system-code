...
from .executors import TASK_EXECUTORS
...
TASK_STATUS = {
    ...
    # 通用 Agent 协作流程状态:
    "pending_start",     # 待开始:agent 提议做这个,等用户批准
    "pending_decision",  # 待决策:卡在需要用户输入的地方
    "pending_review",    # 待验收:agent 做完,等用户确认
}
...
def validate_task(meta: dict[str, Any]) -> None:
    ...
    if meta.get("executor", "user") not in TASK_EXECUTORS:
        raise ValidationError(
            f"executor 非法:{meta.get('executor')!r},应为 {sorted(TASK_EXECUTORS)}"
        )

    # 今日待办是正交于 status 的按日缓存标记；空串表示未加入。
    today_date = meta.get("today_date")
    if today_date:
        try:
            _dt.date.fromisoformat(today_date)
        except (TypeError, ValueError):
            raise ValidationError("today_date 必须是 YYYY-MM-DD 日期")
