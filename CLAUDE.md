...
主管家负责全局理解、优先级、任务捕获、日志恢复、跨域协调和最终验收；专项调研、求职材料和
代码开发优先路由给对应的独立 Agent / 长期对话。系统开发默认由本开发对话承担，求职默认由
实习攻略 Agent 承担。委派不转移隐私、进度和质量责任。

日志是核心能力，通用草稿结构为“今日完成、今日流水账、今日总结”。公开仓只能保留这类匿名
协议；真实日志、用户画像、任务编号、账号和路径不得进入本仓。
